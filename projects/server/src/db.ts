...
async function main() {
  const action = process.argv[2] as 'init' | 'seed';
  const app = await NestFactory.create(AppModule, { logger: false });
  if (action == 'init') await init(app);
  else if (action == 'seed') await seed(app);
  await app.close();
}
...
async function seed(app: INestApplication) {
  ...
  while (users.length < 100) {
    ...
    const user = em.create(User, {
      ...
      password: '$2a$10$H.tcAfDT6phzgoU0mUp.reIdVn3gmo6ZuieTSSJM5/BVLyHW1D/4S', // "password"
      ...
      while (tasks.length - prevLength != count) {
        tasks.push(
          em.create(Task, {
            ...
            description: faker.datatype.boolean()
              ? faker.random.words(faker.datatype.number({ min: 10, max: 100 }))
              : undefined,
            isActive: true,
          }),
        );
